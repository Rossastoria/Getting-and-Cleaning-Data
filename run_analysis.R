...
y_data <- rbind(y_train, y_test)
        ...
        ## ----- reassign subject's motion with a label and clean up those labels
y_data$activity <- factor(y_data$activity, levels = activity$V1, labels = activity$V2)
y_data[,2] <- tolower(y_data[,2])
y_data$activity <- gsub("_", " ", y_data$activity)

        ## ----- Rbind subject to data table
...
tidyavg <- ddply(tidy, .(subject, activity), .fun = function(x) {colMeans(x)})
